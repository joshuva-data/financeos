...
/**
 * FinanceOS Dashboard — Redesigned
 *
 * Architecture decisions:
 * - All heavy calculations are wrapped in useMemo to avoid recomputation on every render.
 * - Data flows down through typed interfaces; no prop-drilling beyond one level.
 * - Sub-components (Skeleton, KPICard, SectionHeader, etc.) are defined in this file
 *   so everything ships as one replacement file. They can be extracted to separate
 *   files later without any interface changes.
 * - Chart data is computed once and memoised; chart components receive plain arrays.
 * - All monetary values go through fmtINR so the format is consistent.
 * - Responsive grid: 1-col mobile → 2-col tablet → 3-col laptop → custom large desktop.
 */
...
import {
  ...
  Heart, Zap, FileText, Bot, ArrowRight, Sparkles,
  BanknoteIcon, BarChart3, RefreshCw, Bell,
} from 'lucide-react'
import {
...
// ── Design tokens (mirror globals.css CSS vars for inline use) ─────────────────
const T = {
  bg:        '#0b0d0f',
  card:      '#12161b',
  border:    '#1e252d',
...
/** Donut / ring for Financial Health Score */
...
/** Micro sparkline inside a KPI card */
...
const KPICard = memo(({
  label, value, sub, subPositive, color, sparkData, href, loading,
}: {
  label:        string
  value:        string
  sub?:         string
  ...
  const inner = (
    <div className="rounded-xl p-4 space-y-3 h-full transition-all hover:border-white/10"
...
// ── Range selector used by NW Growth chart ─────────────────────────────────────

type Range = '1M' | '3M' | '6M' | '1Y' | 'All'
const RANGES: Range[] = ['1M', '3M', '6M', '1Y', 'All']

const RangePicker = memo(({ active, onChange }: {
...
export function PremiumDashboard({
  ...
  const LIQUID = useMemo(() => ['savings', 'current', 'salary', 'wallet', 'cash'], [])

  const liquidCash = useMemo(
    () => accounts.filter(a => LIQUID.includes(a.account_type)).reduce((s, a) => s + a.balance, 0),
    [accounts, LIQUID]
  )
  ...
  const monthlyGrossIncome = useMemo(
    () => income.reduce((s, i) => s + i.gross_amount, 0) / 12,
    [income]
  )

  const monthlyNetIncome = useMemo(
    () => income.reduce((s, i) => s + i.net_amount, 0) / 12,
    [income]
  )
  ...
  const healthScore = useMemo(() => {
    let s = 40
    if (liquidCash >= monthlyNetIncome * 3)   s += 15  // emergency fund
    if (debtRatio < 30)                        s += 15  // manageable debt
    if (goals.length > 0)                      s += 10  // has goals
    if (totalInvested > 0)                     s += 10  // investing
    if (tithe.length > 0)                      s +=  5  // giving
    if (receivables.filter(r => r.status === 'overdue').length === 0) s += 5
    return Math.min(100, s)
  }, [liquidCash, monthlyNetIncome, debtRatio, goals.length, totalInvested, tithe.length, receivables])

  // Monthly income & expense trend for cash flow bar chart
  const cashFlowTrend = useMemo(() => FY_MONTHS.map((label, i) => {
    ...
    const exp   = thisMonthTxns
      .filter(t => t.direction === 'debit' && new Date(t.txn_date).getMonth() === (month - 1))
      .reduce((s, t) => s + t.amount, 0)
    return { label, income: inc, expenses: exp, savings: Math.max(0, inc - exp) }
  }), [income, thisMonthTxns])

  // Net worth history with range filtering
  const nwHistory = useMemo(() => {
    const base = snapshots.length > 1
      ...
      : (['6M ago','5M ago','4M ago','3M ago','2M ago','1M ago','Now'] as const).map((label, i) => ({
          label,
          value: Math.max(0, netWorth * (0.80 + i * 0.034)),
        }))

    const cutMap: Record<Range, number> = { '1M': 2, '3M': 4, '6M': 7, '1Y': 13, 'All': 9999 }
    return base.slice(-cutMap[nwRange])
  }, [snapshots, netWorth, nwRange])

  const nwChange = nwHistory.length > 1
    ? nwHistory[nwHistory.length - 1].value - nwHistory[0].value
    : 0
  const nwChangePct = nwHistory.length > 1 && nwHistory[0].value > 0
    ? (nwChange / nwHistory[0].value) * 100
    : 0

  // Expense breakdown for donut
  const expenseByCategory = useMemo(() => {
    const map: Record<string, number> = {}
    thisMonthTxns.filter(t => t.direction === 'debit').forEach(t => {
  ...
  const forecast = useMemo(() => {
    ...
    debts.forEach(d => {
      if (d.next_emi_date && d.emi_amount) {
        ...
        if (days >= 0 && days <= 45) items.push({ label: `${d.lender_name} EMI`, amount: d.emi_amount, date: d.next_emi_date, daysLeft: days, color: T.red })
      }
    })
    insurance.forEach(p => {
      if (p.renewal_date && p.annual_premium) {
        const days = Math.ceil((new Date(p.renewal_date).getTime() - Date.now()) / 86400000)
        if (days >= 0 && days <= 45) items.push({ label: `${p.policy_name} renewal`, amount: p.annual_premium, date: p.renewal_date, daysLeft: days, color: T.gold })
      }
    })
    return items.sort((a, b) => a.daysLeft - b.daysLeft)
  }, [debts, insurance])

  // AI Insights (rule-based, proactive)
  const insights = useMemo(() => {
    const list: { icon: any; color: string; title: string; desc: string; href: string; badge?: string }[] = []

    if (expenseChange > 15)
      list.push({ icon: TrendingDown, color: T.red, badge: 'Alert',
        title: `Spending up ${Math.abs(expenseChange).toFixed(0)}% this month`,
        desc: `₹${fmtINR(thisExpenses)} vs ₹${fmtINR(lastExpenses)} last month. Review your expenses.`,
        href: '/expenses' })

    if (debtRatio > 40)
      list.push({ icon: AlertCircle, color: T.red, badge: 'Warning',
        title: `Debt ratio at ${debtRatio.toFixed(0)}% — high`,
        desc: 'EMIs exceed 40% of income. Consider prepayment or refinancing.',
        href: '/debt' })

    const emergencyMonths = monthlyNetIncome > 0 ? liquidCash / monthlyNetIncome : 0
    if (emergencyMonths < 3)
      list.push({ icon: Shield, color: T.gold, badge: 'Action',
        title: 'Emergency fund below 3 months',
        desc: `You have ${emergencyMonths.toFixed(1)} months covered. Target: ${fmtINR(monthlyNetIncome * 3)}.`,
        href: '/accounts' })

    if (insurance.length === 0)
      list.push({ icon: Shield, color: T.orange, badge: 'Setup',
        title: 'No insurance policies tracked',
        desc: 'Add your health, life, and vehicle policies to get renewal reminders.',
        href: '/insurance' })

    if (goals.length === 0)
      list.push({ icon: Target, color: T.blue, badge: 'Setup',
        title: 'No financial goals set',
        desc: 'Add goals like emergency fund, home, or retirement to track progress.',
        href: '/goals' })
    ...
    if (tithe.length === 0 && monthlyNetIncome > 0)
      list.push({ icon: Heart, color: '#ec4899', badge: 'Reminder',
        ...
        desc: '10% of your net income. Record your giving in Tithe & Giving.',
        href: '/tithe' })

    const nearestEMI = forecast[0]
    if (nearestEMI && nearestEMI.daysLeft <= 5)
      list.push({ icon: Calendar, color: T.red, badge: 'Urgent',
        title: `EMI due in ${nearestEMI.daysLeft} day${nearestEMI.daysLeft !== 1 ? 's' : ''}`,
        desc: `${nearestEMI.label}: ${fmtINR(nearestEMI.amount)}`,
        href: '/debt' })

    if (list.length === 0)
      list.push({ icon: Sparkles, color: T.green, badge: 'Great',
        title: 'Your finances look healthy',
        desc: 'Keep maintaining your savings rate and debt levels.',
        href: '/dashboard' })

    return list.slice(0, 4)
  }, [expenseChange, thisExpenses, lastExpenses, debtRatio, monthlyNetIncome, liquidCash,
      insurance.length, goals.length, tithe.length, forecast])
  ...
  const actionItems = useMemo(() => {
    ...
    if (accounts.length === 0)
      items.push({ icon: Wallet, color: T.blue, label: 'Link your first account',
        desc: 'Add bank accounts to track balance', href: '/accounts' })
    if (goals.length === 0)
      items.push({ icon: Target, color: T.purple, label: 'Set a financial goal',
        desc: 'Start saving toward something', href: '/goals' })
    items.push({ icon: Zap, color: T.gold, label: 'Upload a document',
      desc: 'Sync salary slip, Form 16, or policy', href: '/automation' })
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: T.muted }}>
            ...
            {' '}{userName.split(' ')[0]}. Here's your financial picture.
          </p>
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          <div className="col-span-2 sm:col-span-3 xl:col-span-2 rounded-xl p-5 space-y-3"
            ...
            <p className="text-3xl font-bold tabular-nums tracking-tight" style={{ color: T.text }}>
              {fmtINR(netWorth)}
            </p>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs" style={{ color: nwChange >= 0 ? T.green : T.red }}>
                {nwChange >= 0 ? '+' : ''}{fmtINR(nwChange)} ({nwChangePct.toFixed(1)}%) this period
              </span>
            </div>
            <ProgressBar
            ...
            <p className="text-[10px]" style={{ color: T.muted }}>
              Assets: {fmtINR(liquidCash + totalInvested)} · Liabilities: {fmtINR(totalDebt)}
            </p>
          </div>

          <KPICard label="Cash Available"   value={fmtINR(liquidCash)}
            sub={`${accounts.filter(a => ['savings','current','salary'].includes(a.account_type)).length} accounts`}
          ...
          <KPICard label="Monthly Savings"  value={fmtINR(monthlySavings)}
            sub={monthlyNetIncome > 0 ? `${((monthlySavings/monthlyNetIncome)*100).toFixed(0)}% savings rate` : undefined}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="xl:col-span-2 rounded-xl p-5 space-y-4"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-2xl font-bold tabular-nums -mt-3" style={{ color: T.text }}>
                {fmtINR(netWorth)}
              </p>
              <p className="text-xs mt-0.5"
                style={{ color: nwChange >= 0 ? T.green : T.red }}>
                {nwChange >= 0 ? '+' : ''}{fmtINR(nwChange)} ({nwChangePct.toFixed(1)}%)
              </p>
            </div>
        ...
        {/* 3b. Financial Health Score + AI Brief */}
        <div className="space-y-4">
          <div className="rounded-xl p-5" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
            ...
            <div className="mt-4 space-y-2">
              {[
                { label: 'Emergency Fund', value: Math.min(100, (liquidCash / (monthlyNetIncome * 6)) * 100), color: T.blue },
                { label: 'Debt Ratio',     value: Math.max(0, 100 - debtRatio * 2),                          color: T.green },
                { label: 'Goal Progress',  value: goals.length > 0 ? Math.round(goals.reduce((s, g) => s + (g.current_amount / g.target_amount), 0) / goals.length * 100) : 0, color: T.purple },
              ].map(item => (
          ...
          <div className="rounded-xl p-4 space-y-2"
            ...
            <p className="text-xs leading-relaxed" style={{ color: T.muted }}>
              {healthScore >= 75
                ? `Your net worth is ${fmtINR(netWorth)}. Savings rate is ${monthlyNetIncome > 0 ? ((monthlySavings/monthlyNetIncome)*100).toFixed(0) : 0}%. Keep it up.`
                : debtRatio > 40
                ? `Debt-to-income ratio is high at ${debtRatio.toFixed(0)}%. Consider accelerating loan repayments.`
                : liquidCash < monthlyNetIncome * 3
                ? `Emergency fund covers ${monthlyNetIncome > 0 ? (liquidCash/monthlyNetIncome).toFixed(1) : 0} months. Build it to 3–6 months.`
                : `You have ${fmtINR(monthlySavings)} in savings this month. Consider putting it into an investment.`
              }
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 rounded-xl p-5"
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={cashFlowTrend} margin={{ top: 0, right: 5, bottom: 0, left: -20 }}
                barSize={6} barGap={2}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" vertical={false} />
          ...
          <div className="rounded-xl p-5 space-y-4"
            ...
            <div className="space-y-3">
              {[
                { label: 'Income',   value: fmtINR(monthlyNetIncome), color: T.green,  icon: TrendingUp,   sub: 'Net take-home'          },
                { label: 'Expenses', value: fmtINR(thisExpenses),     color: T.red,    icon: TrendingDown, sub: `${expenseChange >= 0 ? '+' : ''}${expenseChange.toFixed(0)}% vs last` },
                { label: 'Savings',  value: fmtINR(monthlySavings),   color: T.blue,   icon: BanknoteIcon, sub: monthlyNetIncome > 0 ? `${((monthlySavings/monthlyNetIncome)*100).toFixed(0)}% rate` : '—' },
                { label: 'EMIs',     value: fmtINR(totalEMI),         color: T.orange, icon: CreditCard,   sub: 'Fixed obligations'       },
              ].map(item => (
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-bold tabular-nums flex-shrink-0" style={{ color: T.text }}>
                    {item.value}
                  </p>
                </div>
              ))}
            ...
            {monthlyNetIncome > 0 && (
              <div className="pt-2 border-t" style={{ borderColor: T.border }}>
                <div className="flex justify-between text-[11px] mb-1.5">
                  ...
                  <span style={{ color: T.text }}>{((monthlySavings/monthlyNetIncome)*100).toFixed(0)}%</span>
                </div>
                <ProgressBar value={(monthlySavings/monthlyNetIncome)*100} color={T.green} />
                <p className="text-[10px] mt-1" style={{ color: T.muted }}>Target: ≥ 20% of income</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl p-5" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
          ...
          {expenseByCategory.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 gap-3">
              ...
              <p className="text-sm" style={{ color: T.muted }}>No expenses recorded this month</p>
              <Link href="/expenses"
          ...
          <div className="space-y-2">
            {insights.map((ins, i) => (
              <Link key={i} href={ins.href}
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = `${ins.color}30`; (e.currentTarget as HTMLElement).style.backgroundColor = `${ins.color}08` }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = T.border;          (e.currentTarget as HTMLElement).style.backgroundColor = 'rgba(255,255,255,0.02)' }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    <p className="text-xs font-semibold" style={{ color: T.text }}>{ins.title}</p>
                    {ins.badge && <Badge label={ins.badge} color={ins.color} />}
                  </div>
                  <p className="text-[11px] leading-relaxed" style={{ color: T.muted }}>{ins.desc}</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
        ...
        <div className="rounded-xl p-5" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
          ...
          {forecast.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2">
              ...
              <p className="text-xs text-center" style={{ color: T.muted }}>No bills due in the next 45 days</p>
            </div>
          ) : forecast.map((f, i) => (
            <div key={i} className="flex items-center justify-between py-2.5 border-b"
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className={cn('h-2 w-2 rounded-full flex-shrink-0')}
                  style={{ backgroundColor: f.daysLeft <= 3 ? T.red : f.daysLeft <= 7 ? T.gold : T.muted }} />
                <div className="min-w-0">
                  <p className="text-xs font-medium truncate" style={{ color: T.text }}>{f.label}</p>
                  <p className="text-[10px]" style={{ color: f.daysLeft <= 3 ? T.red : T.muted }}>
                    {f.daysLeft === 0 ? 'Due today' : f.daysLeft === 1 ? 'Due tomorrow' : `In ${f.daysLeft} days`}
                  </p>
                </div>
              </div>
              <p className="text-sm font-bold tabular-nums flex-shrink-0" style={{ color: T.text }}>
            ...
            <div className="mt-4 pt-3 border-t" style={{ borderColor: T.border }}>
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-bold tabular-nums" style={{ color: T.green }}>
                  {fmtINR(Math.max(0, liquidCash - forecast.reduce((s, f) => s + f.amount, 0)))}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="rounded-xl p-5" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
          ...
          {liquidCash + totalInvested === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2">
              <p className="text-xs" style={{ color: T.muted }}>Add accounts and investments to see allocation</p>
                  ...
                  <Link key={item.label} href={item.href} className="block space-y-1.5">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium tabular-nums" style={{ color: T.text }}>
                        {fmtINR(item.value)} <span style={{ color: T.muted }}>({pct}%)</span>
                      </span>
                    </div>
        ...
        <div className="rounded-xl p-5" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
          ...
          <div className="space-y-2">
            {actionItems.map((item, i) => (
              <Link key={i} href={item.href}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium truncate" style={{ color: T.text }}>{item.label}</p>
          ...
          {/* Automation status */}
          <div className="mt-4 pt-3 border-t" style={{ borderColor: T.border }}>
            <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: T.muted }}>
              Automation
            </p>
            <div className="flex items-center gap-2">
              <span className="h-2 w-2 rounded-full animate-pulse" style={{ backgroundColor: T.green }} />
              <span className="text-[11px]" style={{ color: T.muted }}>All systems running</span>
            ...
            <div className="space-y-2">
              {accounts.slice(0, 4).map(acc => (
                <Link key={acc.id} href="/accounts"
                  className="flex items-center justify-between py-2 border-b transition-opacity hover:opacity-80"
                  style={{ borderColor: T.border }}>
                  <div className="min-w-0">
                    <p className="text-xs font-medium truncate" style={{ color: T.text }}>{acc.name}</p>
                    <p className="text-[10px]" style={{ color: T.muted }}>
              ...
              {accounts.length > 4 && (
                <Link href="/accounts" className="text-[11px] block pt-1" style={{ color: T.blue }}>
              ...
              {goals.slice(0, 3).map(goal => {
                ...
                return (
                  <div key={goal.id} className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-medium truncate" style={{ color: T.text }}>{goal.name}</span>
                    ...
                    <div className="flex justify-between text-[10px]">
                      <span style={{ color: T.muted }}>{fmtINR(goal.current_amount)} saved</span>
                      <span style={{ color: T.muted }}>{fmtINR(goal.target_amount)} target</span>
                    </div>
                  </div>
                )
              })}
              ...
              {receivables.filter(r => r.status === 'overdue').slice(0, 2).map(r => (
                <Link key={r.id} href="/receivables"
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-medium truncate" style={{ color: T.text }}>{r.from_name}</p>
                    <p className="text-[10px]" style={{ color: T.red }}>Overdue · {fmtINR(r.balance_due)}</p>
                  </div>
                </Link>
              ))}
      ...
      <div className="rounded-xl p-4" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {[
            ...
            { label: '+ Investment',  href: '/investments',color: '#00C896'},
            { label: '+ Goal',        href: '/goals',      color: T.purple },
            { label: '↑ Upload Doc',  href: '/automation', color: T.muted  },
            { label: '🤖 Ask AI',     href: '/ai-copilot', color: T.blue   },
          ].map(action => (
            <Link key={action.label} href={action.href}
              className="px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all flex-shrink-0"
              style={{
                border:           `1px solid ${action.color}30`,
                color:             action.color,
                backgroundColor:  `${action.color}08`,
              }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.backgroundColor = `${action.color}18` }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.backgroundColor = `${action.color}08` }}>
              {action.label}
            </Link>
          ))}
        </div>
