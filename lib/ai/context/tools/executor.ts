...
import { calculateTithe } from '@/lib/calculations/titheCalculator'
import { getNetWorthBreakdown } from '@/lib/calculations/networth'
...
export async function executeTool(
  ...
  switch (toolName) {
    case 'get_net_worth':
      return executeGetNetWorth(supabase, userId, input)
    case 'get_insurance_coverage':
      return executeGetInsurance(supabase, userId, input)
    case 'get_upcoming_renewals':
      return executeGetRenewals(supabase, userId, input)
    case 'calculate_tax_estimate':
      return executeCalculateTax(supabase, userId, input)
    case 'get_debt_summary':
      return executeGetDebt(supabase, userId, input)
    case 'get_receivables':
      return executeGetReceivables(supabase, userId, input)
    case 'get_expense_analysis':
      return executeGetExpenses(supabase, userId, input)
    case 'calculate_tithe':
      return executeCalculateTithe(supabase, userId, input)
    case 'get_corporate_benefits':
      return executeGetBenefits(supabase, userId)
    case 'get_investment_portfolio':
      return executeGetInvestments(supabase, userId, input)
    case 'get_cash_flow':
      return executeGetCashFlow(supabase, userId, input)
    case 'get_rental_status':
      return executeGetRental(supabase, userId)
    default:
...
async function executeGetNetWorth(
  supabase: Client,
  userId: string,
  input: ToolInput
): Promise<ToolResult> {
  ...
  const result: ToolResult = {
    ...
    liabilities: {
      ...
      accounts: (debt.data ?? []).map(d => ({ name: d.lender_name, type: d.debt_type, outstanding: d.outstanding })),
    },
  }
...
async function executeGetInsurance(
  supabase: Client,
  userId: string,
  input: ToolInput
): Promise<ToolResult> {
  ...
  if (input.type_filter && input.type_filter !== 'all') {
    query = query.eq('insurance_type', input.type_filter as string)
  }
...
async function executeGetRenewals(
  supabase: Client,
  userId: string,
  input: ToolInput
): Promise<ToolResult> {
  ...
  const [insurance, debts, events] = await Promise.all([
    supabase.from('insurance_policies')
      .select('policy_name, insurer_name, renewal_date, annual_premium, insurance_type')
      .eq('user_id', userId).eq('status', 'active').lte('renewal_date', cutoff),
    supabase.from('debt_accounts')
      .select('lender_name, debt_type, emi_amount, next_emi_date')
      .eq('user_id', userId).eq('is_active', true).lte('next_emi_date', cutoff),
    supabase.from('calendar_events')
      .select('title, event_type, event_date, amount')
...
async function executeCalculateTax(
  ...
  const { calculateTax } = await import('@/lib/calculations/taxCalculator')
...
async function executeGetDebt(
  ...
  const { data: income } = await supabase
    ...
    .select('net_amount')
    .eq('user_id', userId)
    .eq('financial_year', getCurrentFY())
  ...
  return {
    ...
    debts: list.map(d => ({
      ...
      next_due: d.next_emi_date,
      remaining_months: d.remaining_months,
    })),
  }
}

async function executeGetReceivables(
  supabase: Client,
  userId: string,
  input: ToolInput
): Promise<ToolResult> {
  let query = supabase
    .from('receivables')
    .select('from_name, from_type, amount, balance_due, due_date, reason, status')
    .eq('user_id', userId)
...
async function executeGetExpenses(
  ...
  let query = supabase
    .from('transactions')
    .select('amount, category, subcategory, txn_date, description')
    .eq('user_id', userId)
    .eq('txn_type', 'expense')
...
async function executeCalculateTithe(
  supabase: Client,
  userId: string,
  input: ToolInput
): Promise<ToolResult> {
  ...
  return {
    ...
    tithe_due: titheDue,
    already_given: totalGiven,
    balance_to_give: balance,
    is_current: balance <= 0,
  }
}

async function executeGetBenefits(
  supabase: Client,
  userId: string
): Promise<ToolResult> {
  ...
  return {
    ...
    epf: {
      employee_contrib: data.epf_employee_contrib,
      employer_contrib: data.epf_employer_contrib,
      balance: data.epf_balance,
      uan: data.uan_number,
    },
    gratuity: {
      eligible: data.gratuity_eligible,
      years_of_service: data.years_of_service,
      estimated: data.estimated_gratuity,
    },
    insurance: {
      health_cover: data.corporate_health_cover,
      life_cover: data.corporate_life_cover,
    },
    bonuses: {
      annual: data.annual_bonus,
      joining: data.joining_bonus,
      retention: data.retention_bonus,
      variable: data.variable_pay,
    },
    benefits: {
      lta_balance: data.lta_balance,
      medical_reimbursement: data.medical_reimbursement,
      learning_budget: data.learning_budget,
      learning_used: data.learning_used,
      learning_remaining: data.learning_budget - data.learning_used,
    },
    leave: {
      total: data.total_leaves,
      taken: data.leaves_taken,
      remaining: (data.total_leaves ?? 0) - (data.leaves_taken ?? 0),
    },
  }
}

async function executeGetInvestments(
  supabase: Client,
  userId: string,
  input: ToolInput
): Promise<ToolResult> {
  const { data, error } = await supabase
    .from('investments')
    .select('name, investment_type, invested_amount, current_value, unrealized_pnl, xirr, broker, is_tax_saving')
    .eq('user_id', userId)
  ...
  return {
    ...
    allocation: Object.entries(grouped).map(([name, vals]) => ({
      ...
      count: vals.count,
      share_pct: totalCurrent > 0 ? Math.round((vals.current / totalCurrent) * 100) : 0,
    })),
    holdings: list,
  }
}

async function executeGetCashFlow(
  supabase: Client,
  userId: string,
  input: ToolInput
): Promise<ToolResult> {
...
async function executeGetRental(
  supabase: Client,
  userId: string
): Promise<ToolResult> {
  const [properties, payments] = await Promise.all([
    ...
    supabase.from('rental_payments')
      .select('amount_due, amount_paid, status, tenant_id, month, year')
      .eq('user_id', userId)
